...
// ─── i18n ──────────────────────────────────────────────────────────────────────
// A from-scratch, dependency-free string-lookup system. Strings live in
// TRANSLATIONS.en keyed by a short id; t(key) looks up the string, falling
// back to the key itself if one is ever missing (so a missed entry shows an
// obvious raw key instead of a blank/broken label). English-only — Mandarin
// support was removed.
//
// Adding a new translatable string: pick a key, add it to the "en" object
// below, then call t("yourKey") in the component instead of a literal string.
const TRANSLATIONS = {
  en: {
    // Login screen
    username: "Username",
...
// Only "en" is ever valid now that Mandarin support has been removed —
// no localStorage read/write needed for a single-language app.
